<x-admin-layout title="Editar Trilha">
    <div class="mb-6">
    ...
    <div class="bg-white rounded-lg shadow">
        <div class="p-6 border-b border-gray-200">
            <h2 class="text-lg font-medium text-gray-900">Editar Trilha: {{ $category->name }}</h2>
        </div>
        ...
        <form action="{{ route('admin.categories.update', $category) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            ...
            <div>
                ...
                <div id="cover-upload-area" class="relative border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-blue-400 transition-colors cursor-pointer">
                    <div id="cover-placeholder" class="{{ $category->cover_image ? 'hidden' : '' }} space-y-2">
                    ...
                    <div id="cover-preview" class="{{ $category->cover_image ? '' : 'hidden' }}">
                        <img id="cover-preview-img"
                             src="{{ $category->cover_image ? asset('storage/' . $category->cover_image) : '' }}"
                             alt="Preview"
    ...
    <!-- Seção de Cursos da Trilha -->
    <div class="bg-white rounded-lg shadow mt-6">
        <div class="p-6 border-b border-gray-200 flex items-center justify-between">
            <div>
                <h2 class="text-lg font-medium text-gray-900">Cursos desta Trilha</h2>
                <p class="text-sm text-gray-500">{{ $category->courses->count() }} curso(s) vinculado(s)</p>
            </div>
                    ...
                    <div class="p-4 flex items-center justify-between hover:bg-gray-50">
                        <div class="flex items-center gap-3 flex-1 min-w-0">
                            @if($course->cover_image)
                                <img src="{{ asset('storage/' . $course->cover_image) }}" alt="{{ $course->title }}" class="w-16 h-9 rounded object-cover flex-shrink-0">
                            @else
                            ...
                            <div class="min-w-0">
                                ...
                                <div class="text-xs text-gray-500">{{ $course->lessons_count }} aula(s)</div>
                            </div>
                        </div>
                        <div class="flex items-center gap-3 flex-shrink-0 ml-4">
                            ...
                            @endif
                            <a href="{{ route('admin.courses.edit', $course) }}" class="text-sm text-yellow-600 hover:text-yellow-900">Editar</a>
                        </div>
                    </div>
                @endforeach
            </div>
        @else
            <div class="p-8 text-center">
                ...
                <p class="mt-2 text-sm text-gray-500">Nenhum curso nesta trilha ainda.</p>
                <a href="{{ route('admin.courses.create') }}?category_id={{ $category->id }}"
                   class="mt-3 inline-block text-sm text-blue-600 hover:text-blue-800">
                    Criar o primeiro curso &rarr;
                </a>
            </div>
        @endif
    </div>
